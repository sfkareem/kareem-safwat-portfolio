...
function ArrowIcon({ className }: { className?: string }) {
  return (
    <div className={cn("group flex size-full cursor-pointer items-center justify-center", className)}>
      <div className="relative grid cursor-pointer items-center justify-center">
        <ChevronRight className="size-full transition-all duration-500 ease-out group-hover:translate-x-0.5" />
...
function MenuIcon({ className, open, onToggle }: { className?: string; open?: boolean; onToggle?: () => void }) {
  const [internalToggle, setInternalToggle] = useState(false);
  const toggle = open !== undefined ? open : internalToggle;

  const handleClick = () => {
    if (open !== undefined && onToggle) {
      onToggle();
    } else if (open === undefined) {
      setInternalToggle((x) => !x);
    }
  };
