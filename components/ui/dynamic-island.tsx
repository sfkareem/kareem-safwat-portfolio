...
const DynamicIslandProvider: React.FC<DynamicIslandProviderProps> = ({
  ...
  const contextValue = { state, dispatch, setSize, scheduleAnimation, presets: DynamicIslandSizePresets }
...
const calculateDimensions = (size: SizePresets, screenSize: string, currentSize: Preset): { width: string; height: number } => {
  ...
  return (
    <motion.div
      ...
      animate={{
        ...
        clipPath: `none`,
        transitionEnd: { clipPath: `url(#squircle-${state.size})` },
      }}
      style={{ willChange }}
      {...props}
    >
