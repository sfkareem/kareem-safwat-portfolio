...
export const ShareQRCode = ({ url }: ShareQRCodeProps) => {
  ...
  const downloadSVG = () => {
    if (qrCode) {
      qrCode.download({ extension: "svg", name: "business-card-qr" });
    }
  };

  const downloadPNG = () => {
    if (qrCode) {
      qrCode.download({ extension: "png", name: "business-card-qr" });
    }
  };

  return (
    <Card className="flex w-full max-w-[500px] flex-col rounded-[14px] bg-muted/40 p-[4px] shadow-none border-none">
      <CardContent className="relative flex flex-col items-center justify-center space-y-6 overflow-hidden rounded-[10px] bg-background p-6 ring-1 ring-border">
        ...
        <div className="flex w-full items-center justify-center rounded-lg border-2 border-dashed py-6">
          <div className="relative p-0.5">
            ...
            <div className="absolute top-0 left-0 size-3 rounded-tl border-t-2 border-l-2 border-foreground"></div>
            <div className="absolute top-0 right-0 size-3 rotate-90 rounded-tl border-t-2 border-l-2 border-foreground"></div>
            <div className="absolute bottom-0 left-0 size-3 -rotate-90 rounded-tl border-t-2 border-l-2 border-foreground"></div>
            <div className="absolute right-0 bottom-0 size-3 rotate-180 rounded-tl border-t-2 border-l-2 border-foreground"></div>
          </div>
        </div>
        <div className="flex w-full items-center justify-between rounded-xl bg-muted p-3.5">
          <div className="flex items-center gap-1.5 overflow-hidden">
            ...
            <span className="cursor-pointer text-sm font-medium tracking-[-0.006em] hover:underline truncate" onClick={copyToClipboard}>
              {url}
            </span>
          </div>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button className="size-7 shrink-0" size="icon" variant="ghost" onClick={copyToClipboard}>
                  {copied ? (
                    <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" className="w-4 h-4 text-green-500">
